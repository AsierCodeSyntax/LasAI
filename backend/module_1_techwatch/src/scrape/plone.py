...
def _extract_from_jsonld(soup: bs4.BeautifulSoup) -> Optional[datetime]:
    """
    Searches for datePublished/dateModified in JSON-LD.
    Plone usually includes schema.org JSON-LD in many pages.
    """
...
def _find_date_in_jsonld(data: Any) -> Optional[datetime]:
    """
    data can be dict or list. We search for datePublished / dateModified recursively.
    """
    ...
    if isinstance(data, dict):
        ...
        # sometimes it comes inside "@graph"
        if "@graph" in data:
            return _find_date_in_jsonld(data["@graph"])

        # recursive over values
...
def _extract_from_visible_text(soup: bs4.BeautifulSoup) -> Optional[datetime]:
    """
    Last fallback: search for a date in the visible text.
    Useful if Plone shows "Effective date: 2024-..." or similar.
    """
    ...
    # Also try “Month DD, YYYY” in English (very common)
    m2 = re.search(
...
def discover_plone_news_events(listing_html: str, base_url: str) -> List[DiscoveredLink]:
    ...
    for a in soup.select("a[href]"):
        ...
        # links under news-and-events (avoids the listing itself)
