...
def update_source_fetched(conn: psycopg.Connection, source_id: str) -> None:
    """
    For scrapes without a reliable date: DO NOT touch last_published_at.
    """
    with conn.cursor() as cur:
        cur.execute(
            """
...
def main() -> None:
    ...
    ap.add_argument("--topic", required=True, help="The ID of the topic to ingest via scrape")
    ...
    with psycopg.connect(args.db) as conn:
        ...
        if not scrape_sources:
            print(f"No scrape sources enabled for topic {args.topic}.")
            return
        ...
        for src in scrape_sources:
            ...
            for link in links[:100]:  # cap
                ...
                # If there is no real published_at, use fetched_at to allow for a weekly window.
