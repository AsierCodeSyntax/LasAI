...
def is_mapping_like(obj: Any) -> bool:
    # FeedParserDict has .get/.keys but no always is dict
...
def load_sources_yaml(path: str) -> Dict[str, Any]:
    """Load the main sources.yaml and merge it with sources_ia.yaml if it exists."""
    # 1. Load the main (sources.yaml)
    if not os.path.exists(path):
        print(f"⚠️ Main file not found: {path}")
        fuentes_totales = {"topics": {}}
    else:
        with open(path, "r", encoding="utf-8") as f:
            fuentes_totales = yaml.safe_load(f) or {"topics": {}}

    # 2. Load the IA (sources_ia.yaml)
    # We assume it's in the same directory as the main file
    base_dir = os.path.dirname(os.path.abspath(path))
    ia_path = os.path.join(base_dir, "sources_ia.yaml")

    if os.path.exists(ia_path):
        print(f"🤖 [Ingest] Merging IA-discovered sources from: {ia_path}")
        with open(ia_path, "r", encoding="utf-8") as f:
            fuentes_ia = yaml.safe_load(f) or {}

        # Merge the topics
        for topic, contenido in fuentes_ia.items():
            # Ensure structure in the main dictionary
            ...
            # Add IA sources (come in reduced format, we expand them)
            if "rss" in contenido:
                for rss_ia in contenido["rss"]:
                    url_ia = rss_ia.get("url")
                    if url_ia and url_ia not in urls_existentes:
                        # Convert the simple format of the Scout to the complex format of ingest.py
                        nueva_fuente = {
...
def main() -> None:
    ...
    ap.add_argument("--topic", required=True, help="The topic ID to ingest (e.g., plone, ciberseguridad, etc.)")
    ...
    if not yaml_sources:
        print(f"No sources found for topic='{args.topic}' in sources.yaml nor sources_ia.yaml")
        return
    ...
    with psycopg.connect(args.db) as conn:
        ...
        for src in rss_sources:
            ...
            import requests
            try:
                req_headers = {"User-Agent": "TechWatchBot/1.0"}
                ...
                print(f"  ❌ Network error or Timeout ignored: {e}")
                update_source_fetch_only(conn, source_id)
                conn.commit()
                continue
